...
The formula string, ``"unemployment + hourly_earn"`` tells GAUSS to load just the two specified variables from the ``detroit`` file.
...
// Preview the xle data
    head(xle);

In the code above the ``"."` in the formula string specifies to load all variables and the ``" -date"`` tell GAUSS to exclude the ``date`` variable. This means GAUSS will load all variables *except*  ``date``.     
