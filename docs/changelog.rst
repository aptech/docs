...
#. New feature: 3D surface graphs are canvas items. :func:`plotLayout` and :func:`plotCustomLayout` place a surface in its cell beside 2D graphs, and a surface can be moved, resized, annotated with text and shapes, popped out and exported like any other graph.
#. New feature: 3D surface graphs have two new settings, **Projection** (Orthographic or Perspective) and **Box** (Fill the cell or Cube), on the Surface Only page of the Graphics preferences and in the graph settings tree. They are saved in ``.plot`` files.
#. New feature: Two new camera angle presets for 3D surface graphs, **Front Left** and **Front Right**, look at the surface from a little in front of the isometric views (37.5 degrees around and 30 degrees above).
#. Enhanced functionality: 3D surface graphs have a new look: axis labels and the title stay sharp at any display scaling, the axis box and grid are cleaner, and text is real text in PDF and SVG exports.
#. Enhanced functionality: The mouse rotation of a 3D surface is kept across settings changes and saved in ``.plot`` files. The graph settings show it as **Rotation** and **Tilt**; choosing a camera angle preset returns to the preset.
...
#. Behavior change: 3D surface graphs are drawn with orthographic projection (parallel edges stay parallel), the box fills the plot area, faces are not shaded (the **Lighting** setting is now 0 by default; raise it for shaded faces, 5 gives the previous look), axis limits end at the nearest tick with tick steps of 1, 2 and 5, and the axis rulers sit on the two floor edges nearest the viewer and on the left-most vertical edge. The new **Projection** and **Box** settings restore a perspective lens or a cube-shaped box.
#. Behavior change: The **Below** camera angle presets for 3D surfaces now look from below; they were identical to the **Low** presets. A GAUSS 26 ``.plot`` file with a surface opens as before; a GAUSS 27 file with a surface beside other graphs shows only the surface in GAUSS 26.
#. Behavior change: Adding a 2D curve to a surface graph, or a surface to a 2D graph, opens a new graph.
