...
.. function:: dynOut = dynFacBAM(data, dctl)
    ...
    :param data: (NumSubGroups*NumObs x (3+k)), a stacked matrix of observable data. It must include a data/observation indicator in the first column, a group indicator in the second column, and a sub-group indicator in the third column. Unstacked panel series data can be converted to an acceptable format using the :func:`createDynFacData` procedure.
    :type data: Matrix
